...
filter_data <- raw_data %>%
  ...
  filter(!str_count(`protein`, "sp\\|")>1) %>%   #remove entries with multiple protein mappings
  filter(!str_count(protein, "tr\\|")>1) %>%
...
aggregate_data_2 <- filter_data %>%  group_by(protein) %>% 
  summarise("protein" = paste(`protein`, collapse = "~"),"mean protein probability"= mean(`protein probability`), 
            ...
            "mean LIBRA number peptides" = mean(`LIBRA number peptides`), "peptides"=paste(`peptides`, collapse = "|"), 
            "mean peptides" = mean(`num peptides`), "max_peptides"= max(`num peptides`)) %>%
...
mutate_aggregate_data <- mutate_aggregate_data %>%
  mutate(species=str_extract_all(`protein description`, "OS=.+ OX=")) %>%
  mutate_at("species", str_remove_all,"OS=") %>%
  mutate_at("species", str_remove_all,"OX=") %>%
  mutate(tax_id=str_extract(`protein description`,"(?<=OX=)[[:digit:]]+(=?)")) %>%
  mutate(accession=str_extract(protein, "(?<=\\|).*(?=\\|)")) %>%
  relocate(any_of(c("species","tax_id","accession")), .after = `protein description`)
  
mutate_aggregate_data <- mutate_aggregate_data %>%
  mutate(healthy_obs=rowSums(!is.na(mutate_aggregate_data[,c(17:39)]))) %>%
  mutate(tumour_obs=rowSums(!is.na(mutate_aggregate_data[,c(40:88)])))

filter_db <- read.csv("") #Import CD-HIT output detailing 90% identity clusters
total_data <- merge(mutate_aggregate_data,filter_db, by = "accession")
total_filter_data <- total_data %>%
  filter(n==1) %>%                    # filter out proteins that have 90% clusters
  filter(max_peptides>=2)             # filter out proteins identified by only 1 peptide
  

write.csv(total_filter_data, row.names = FALSE, file = "output.csv")
