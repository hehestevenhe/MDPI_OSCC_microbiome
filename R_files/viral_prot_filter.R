...
mutate_aggregate_data <- mutate_aggregate_data %>%
  ...
  relocate(any_of(c("species","tax_id")), .after = `protein description`)
  
mutate_aggregate_data <- mutate_aggregate_data %>%
  mutate(healthy_obs=rowSums(!is.na(mutate_aggregate_data[,c(16:38)]))) %>%
  mutate(tumour_obs=rowSums(!is.na(mutate_aggregate_data[,c(39:87)])))
